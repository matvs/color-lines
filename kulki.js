...
function Rysuj_plansze() {
    ...
    var i = 0;
    while (form.elements[i].checked != true)
        ++i;
...
function szukaj_sciezki(xp, yp) {
    ...
    if (kolka[(yp - 50) / (bok + 5)][(xp - 50) / (bok + 5)].ruch == 8) {
        if (xp == px && yp == py)
            ...
            return szukaj_sciezki(((kolka[(yp - 50) / (bok + 5)][(xp - 50) / (bok + 5)].root.x) - bok / 2), ((kolka[(yp - 50) / (bok + 5)][(xp - 50) / (bok + 5)].root.y) - bok / 2));
    }
...
function los(ile) {
    ...
    while (i < ile) {
        ...
        for (var yy = 50; (!koniec) && yy <= 490; yy += bok + 5)
            for (var xx = 50; xx <= 490; xx += bok + 5)
                if (x >= xx && x <= xx + (bok + 5) && y >= yy && y <= yy + (bok + 5)) {
                    x = xx;
                    y = yy;
                    koniec = true;
                    break;
        ...
        if ((kolka[(y - 50) / (bok + 5)][(x - 50) / (bok + 5)]).widoczny == false) {
            if (level > 1)
                (kolka[(y - 50) / (bok + 5)][(x - 50) / (bok + 5)]).kolor = kolory_next[i];
    ...
    if (level > 1) {
        los_kolory("trzy");
        kolory_next[0] = kolory_next[3];
        kolory_next[1] = kolory_next[4];
        kolory_next[2] = kolory_next[5];
        wyswietl_nastepne();
    }
}
...
function punkty() {
    ...
    for (var i = 0; (!koniec) && i < 9; ++i)
        for (var j = 0; j < 9; ++j) {
            if (kolka[i][j].widoczny == true) {
                ...
                poczatek_poziom = j;
                ++j;
                while (j < 9 && kolka[i][j].widoczny == true && kolka[i][j].kolor == kolor)
                    ++j;
    ...
    for (var i = 0; (!koniec) && i < 9; ++i)
        for (var j = 0; j < 9; ++j) {
            if (kolka[j][i].widoczny == true) {
                ...
                poczatek_pion = j;
                ++j;
                while (j < 9 && kolka[j][i].widoczny == true && kolka[j][i].kolor == kolor)
                    ++j;
    ...
    for (j = 0; (!koniec) && j < 5; ++j)
        for (var i = 0; i < 9 - j; ++i) {
            if (kolka[i + j][i].widoczny == true) {
                ...
                poczatek_skos_lp = i;
                while (i < 9 - j && kolka[i + j][i].widoczny == true && kolka[i + j][i].kolor == kolor)
                    ++i;
                length_skos_lp = i - poczatek_skos_lp;
                if (length_skos_lp >= 5) {
                    koniec = true;
                    koniec_skos_lp = true;
                    przesuniecie_skos_lp_wier = j;
                    udalo_sie = true;
                    break;
                }
                else {
        ...
        for (j = 0; (!koniec) && j < 5; ++j)
            for (var i = 0; i < 9 - j; ++i) {
                if (kolka[i][i + j].widoczny == true) {
                    ...
                    poczatek_skos_lp = i;
                    while (i < 9 - j && kolka[i][i + j].widoczny == true && kolka[i][i + j].kolor == kolor)
                        ++i;
                    length_skos_lp = i - poczatek_skos_lp;
                    if (length_skos_lp >= 5) {
                        koniec = true;
                        przesuniecie_skos_lp_kol = j;
                        udalo_sie = true;
                        break;
                    }
                    else {
    ...
    for (j = 8; (!koniec) && j >= 4; --j)
        for (var i = 0; i < 9 - (8 - j); ++i) {
            if (kolka[i][j - i].widoczny == true) {
                kolor = kolka[i][j - i].kolor;
                poczatek_skos_pl = i;
                while (i < 9 - (8 - j) && kolka[i][j - i].widoczny == true && kolka[i][j - i].kolor == kolor)
                    ++i;
                length_skos_pl = i - poczatek_skos_pl;
                if (length_skos_pl >= 5) {
                    koniec = true;
                    koniec_skos_pl = true;
                    przesuniecie_skos_pl_kol = j;
                    udalo_sie = true;
                    break;
                }
                else {
        ...
        for (var j = 0; (!koniec) && j < 5; ++j)
            for (var i = j; i < 9; ++i) {
                if (kolka[8 - i + j][i].widoczny == true) {
                    ...
                    poczatek_skos_pl = i;
                    while (i < 9 && kolka[8 - i + j][i].widoczny == true && kolka[8 - i + j][i].kolor == kolor)
                        ++i;
                    length_skos_pl = i - poczatek_skos_pl;
                    if (length_skos_pl >= 5) {
                        koniec = true;
                        przesuniecie_skos_pl_wier = j;
                        udalo_sie = true;
                        break;
                    }
                    else {
    ...
    if (length_poziom >= 5) {
        for (var i = poczatek_poziom; i < poczatek_poziom + length_poziom; ++i)
            kolka[nr_wiersza][i].usun();
        wynik += length_poziom;
        outWynik.innerHTML = wynik;
    }
    if (length_pion >= 5) {
        for (var i = poczatek_pion; i < poczatek_pion + length_pion; ++i)
            kolka[i][nr_kolumny].usun();
        wynik += length_pion;
        outWynik.innerHTML = wynik;
    }
    if (length_skos_lp >= 5) {
        for (var i = poczatek_skos_lp; i < poczatek_skos_lp + length_skos_lp; ++i)
            kolka[i + przesuniecie_skos_lp_wier][i + przesuniecie_skos_lp_kol].usun();
        wynik += length_skos_lp;
        outWynik.innerHTML = wynik;
    }
    if (length_skos_pl >= 5) {
        for (var i = poczatek_skos_pl; i < poczatek_skos_pl + length_skos_pl; ++i)
            kolka[Math.abs(8 * (przesuniecie_skos_pl_wier == 0 ? 0 : 1) + przesuniecie_skos_pl_wier - i)][Math.abs(przesuniecie_skos_pl_kol - i)].usun();
